...
- **用例1**：获取普通用户列表
  - 请求：`GET /api/admin/users/role?role=USER`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回所有普通用户列表

- **用例2**：获取商家用户列表
  - 请求：`GET /api/admin/users/role?role=MERCHANT`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回所有商家用户列表
...
- **用例1**：管理员获取待审核店铺列表
  - 请求：`GET /api/admin/stores/pending`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回所有待审核的店铺列表
  
- **用例2**：使用另一个URL路径获取待审核店铺列表
  - 请求：`GET /api/stores/pending`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回所有待审核的店铺列表
...
- **用例1**：审核通过店铺
  - 请求：`PUT /api/admin/stores/3/status?status=1`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回成功信息，提示店铺审核通过

- **用例2**：关闭店铺
  - 请求：`PUT /api/stores/2/status?status=2`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回成功信息，提示店铺已关闭

- **用例3**：非管理员尝试审核店铺
  - 请求：`PUT /api/stores/3/status?status=1`
  - 请求头：`Authorization: Bearer {普通用户token}`
  - 预期结果：返回403错误，提示无权限
...
- **用例1**：管理员获取客服满意度统计
  - 请求：`GET /api/admin/customer-service/stats`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回所有店铺的客服满意度统计
...
- **用例1**：获取满意度低于4.0的店铺
  - 请求：`GET /api/admin/customer-service/low-rating?threshold=4.0`
  - 请求头：`Authorization: Bearer {管理员token}`
...
- **用例1**：获取指定店铺的客服详情
  - 请求：`GET /api/admin/customer-service/store/1`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回指定店铺的客服满意度详情

- **用例2**：获取不存在店铺的客服详情
  - 请求：`GET /api/admin/customer-service/store/999`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回错误信息，提示店铺不存在
...
- **用例1**：获取客服投诉列表
  - 请求：`GET /api/admin/customer-service/complaints`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：返回所有客服会话投诉列表
...
- **用例1**：处理客服投诉
  - 请求：`POST /api/customer-service/complaints/10`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 请求体：
    ```json
    {
...
- **用例1**：显示公告
  - 请求：`PUT /api/announcements/2/status?status=1`
  - 请求头：`Authorization: Bearer {管理员token}`
  - 预期结果：更新成功，返回true
