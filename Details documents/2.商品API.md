...
- **用例1**：获取第一页商品，每页10条
  - 请求：`GET /api/products/page?pageNum=1&pageSize=10`
  - 预期结果：返回第一页商品列表，每页10条

- **用例2**：获取第二页商品，每页5条
  - 请求：`GET /api/products/page?pageNum=2&pageSize=5`
  - 预期结果：返回第二页商品列表，每页5条

- **用例3**：使用默认分页参数
  - 请求：`GET /api/products/page`
  - 预期结果：返回第一页商品列表，每页10条
...
- **用例1**：获取特定店铺的商品
  - 请求：`GET /api/products/store/1`
  - 预期结果：返回店铺ID为1的所有商品

- **用例2**：获取不存在店铺的商品
  - 请求：`GET /api/products/store/999`
  - 预期结果：返回空数组
...
- **用例1**：获取特定分类的商品
  - 请求：`GET /api/products/category/3`
  - 预期结果：返回分类ID为3的所有商品

- **用例2**：获取不存在分类的商品
  - 请求：`GET /api/products/category/999`
  - 预期结果：返回空数组
...
- **用例1**：使用有效关键词搜索
  - 请求：`GET /api/products/search?keyword=iPhone`
  - 预期结果：返回含有"iPhone"关键词的商品列表

- **用例2**：使用无匹配关键词搜索
  - 请求：`GET /api/products/search?keyword=不存在的产品`
  - 预期结果：返回空数组

- **用例3**：空关键词搜索
  - 请求：`GET /api/products/search?keyword=`
...
- **用例1**：获取默认数量热门商品
  - 请求：`GET /api/products/hot`
  - 预期结果：返回10个热门商品（按销量排序）

- **用例2**：获取指定数量热门商品
  - 请求：`GET /api/products/hot?limit=5`
  - 预期结果：返回5个热门商品（按销量排序）
...
- **用例1**：获取默认数量新品
  - 请求：`GET /api/products/new`
  - 预期结果：返回10个新品（按创建时间排序）

- **用例2**：获取指定数量新品
  - 请求：`GET /api/products/new?limit=5`
  - 预期结果：返回5个新品（按创建时间排序）
...
- **用例1**：商家添加商品
  - 请求：`POST /api/products`（使用商家token）
    ```json
    {
      "name": "小米12",
      "storeId": 3,
...
- **用例2**：管理员添加商品
  - 请求：`POST /api/products`（使用管理员token）
    ```json
    {
      "name": "OPPO Find X5",
      "storeId": 4,
...
- **用例3**：非商家或管理员添加商品
  - 请求：`POST /api/products`（使用普通用户token）
    ```json
    {
      "name": "测试商品",
      "storeId": 1,
...
- **用例1**：更新商品名称和价格
  - 请求：`PUT /api/products/1`（使用商家token）
    ```json
    {
      "name": "iPhone 13 Pro",
      "price": 7999.00
...
- **用例3**：更新不存在的商品
  - 请求：`PUT /api/products/999`（使用商家token）
    ```json
    {
      "name": "测试商品",
      "price": 1000.00
...
- **用例4**：非商家或管理员更新商品
  - 请求：`PUT /api/products/1`（使用普通用户token）
    ```json
    {
      "price": 6000.00
    }
...
- **用例1**：商家删除自己的商品
  - 请求：`DELETE /api/products/1`（使用商品所属商家token）
  - 预期结果：删除成功，返回true

- **用例2**：管理员删除商品
  - 请求：`DELETE /api/products/2`（使用管理员token）
  - 预期结果：删除成功，返回true

- **用例3**：删除不存在的商品
  - 请求：`DELETE /api/products/999`（使用商家token）
  - 预期结果：删除失败，返回商品不存在的错误信息

- **用例4**：非商家或管理员删除商品
  - 请求：`DELETE /api/products/1`（使用普通用户token）
  - 预期结果：返回403错误，提示无权限
