...
- **用例1**：已登录用户获取购物车列表
  - 请求：`GET /api/cart`（携带有效token）
  - 预期结果：返回用户的购物车列表

- **用例2**：未登录用户获取购物车列表
  - 请求：`GET /api/cart`（无token或无效token）
  - 预期结果：返回未授权错误（401）
...
- **用例1**：更新购物车商品数量
  - 请求：`PUT /api/cart/1/quantity?quantity=3`
  - 预期结果：更新成功，返回更新后的购物车项

- **用例2**：更新不存在的购物车项
  - 请求：`PUT /api/cart/999/quantity?quantity=3`
  - 预期结果：更新失败，返回"购物车商品不存在"错误

- **用例3**：更新数量超过库存
  - 请求：`PUT /api/cart/1/quantity?quantity=1000`
...
- **用例1**：更新购物车商品选择状态为未选中
  - 请求：`PUT /api/cart/1/selected?selected=false`
  - 预期结果：更新成功，返回更新后的购物车项，selected为false

- **用例2**：更新购物车商品选择状态为选中
  - 请求：`PUT /api/cart/1/selected?selected=true`
  - 预期结果：更新成功，返回更新后的购物车项，selected为true

- **用例3**：更新不存在的购物车项
  - 请求：`PUT /api/cart/999/selected?selected=true`
  - 预期结果：更新失败，返回"购物车商品不存在"错误
...
- **用例1**：全选购物车商品
  - 请求：`PUT /api/cart/selected?selected=true`
  - 预期结果：更新成功，返回true

- **用例2**：取消全选购物车商品
  - 请求：`PUT /api/cart/selected?selected=false`
  - 预期结果：更新成功，返回true
...
- **用例1**：删除存在的购物车项
  - 请求：`DELETE /api/cart/1`
  - 预期结果：删除成功，返回true

- **用例2**：删除不存在的购物车项
  - 请求：`DELETE /api/cart/999`
  - 预期结果：删除失败，返回"购物车商品不存在"错误

- **用例3**：删除别人的购物车项
  - 请求：`DELETE /api/cart/2`（该项属于其他用户）
  - 预期结果：删除失败，返回"购物车商品不存在"错误
...
- **用例1**：删除已选择的购物车商品
  - 请求：`DELETE /api/cart/selected`
  - 预期结果：删除成功，返回true

- **用例2**：删除已选择的购物车商品（无选中项）
  - 请求：`DELETE /api/cart/selected`（无选中项）
  - 预期结果：操作成功，返回true
...
- **用例1**：获取已选择的购物车商品
  - 请求：`GET /api/cart/selected`
  - 预期结果：返回已选择的购物车商品列表

- **用例2**：获取已选择的购物车商品（无选中项）
  - 请求：`GET /api/cart/selected`（无选中项）
  - 预期结果：返回空数组
...
- **用例1**：获取购物车已选择商品的总价
  - 请求：`GET /api/cart/amount`
  - 预期结果：返回已选择商品的总价和商品数量

- **用例2**：获取购物车已选择商品的总价（无选中项）
  - 请求：`GET /api/cart/amount`（无选中项）
  - 预期结果：返回总价为0，商品数量为0
...
- **用例1**：检查已在购物车中的商品
  - 请求：`GET /api/cart/check?productId=101&specInfo={"color":"红色","size":"L"}`
  - 预期结果：返回true

- **用例2**：检查不在购物车中的商品
  - 请求：`GET /api/cart/check?productId=102`
  - 预期结果：返回false
