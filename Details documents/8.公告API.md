...
- **用例1**：管理员获取所有公告
  - 请求：`GET /api/announcements/admin`
  - 预期结果：返回所有公告列表，包括显示和隐藏的
  
- **用例2**：非管理员获取所有公告
  - 请求：`GET /api/announcements/admin`（使用普通用户token）
  - 预期结果：返回403错误，提示无权限
...
- **用例1**：获取存在的公告
  - 请求：`GET /api/announcements/1`
  - 预期结果：返回ID为1的公告信息

- **用例2**：获取不存在的公告
  - 请求：`GET /api/announcements/999`
  - 预期结果：返回错误信息，提示公告不存在
  
- **用例3**：获取隐藏状态的公告（普通用户）
  - 请求：`GET /api/announcements/3`（假设ID为3的公告状态为隐藏）
  - 预期结果：普通用户返回错误信息
...
- **用例3**：非管理员添加公告
  - 请求：`POST /api/announcements`（使用普通用户token）
    ```json
    {
      "title": "测试公告",
      "content": "测试内容",
...
- **用例4**：非管理员更新公告
  - 请求：`PUT /api/announcements/1`（使用普通用户token）
    ```json
    {
      "title": "测试公告"
    }
...
- **用例1**：删除存在的公告
  - 请求：`DELETE /api/announcements/2`
  - 预期结果：删除成功，返回true

- **用例2**：删除不存在的公告
  - 请求：`DELETE /api/announcements/999`
  - 预期结果：删除失败，返回公告不存在的错误信息

- **用例3**：非管理员删除公告
  - 请求：`DELETE /api/announcements/1`（使用普通用户token）
  - 预期结果：返回403错误，提示无权限
...
- **用例1**：将公告设为显示状态
  - 请求：`PUT /api/announcements/3/status?status=1`
  - 预期结果：更新成功，返回true

- **用例2**：将公告设为隐藏状态
  - 请求：`PUT /api/announcements/1/status?status=0`
  - 预期结果：更新成功，返回true

- **用例3**：传入无效的状态值
  - 请求：`PUT /api/announcements/1/status?status=2`
  - 预期结果：更新失败，返回错误信息（状态值无效）

- **用例4**：更新不存在公告的状态
  - 请求：`PUT /api/announcements/999/status?status=1`
  - 预期结果：更新失败，返回公告不存在的错误信息

- **用例5**：非管理员更新公告状态
  - 请求：`PUT /api/announcements/1/status?status=0`（使用普通用户token）
  - 预期结果：返回403错误，提示无权限
...
- **用例1**：标记未读公告为已读
  - 请求：`POST /api/announcements/2/read`（假设用户未读该公告）
  - 预期结果：标记成功，返回true

- **用例2**：再次标记已读公告
  - 请求：`POST /api/announcements/2/read`（假设已经标记为已读）
  - 预期结果：操作成功，返回true（重复标记不报错）

- **用例3**：标记不存在的公告
  - 请求：`POST /api/announcements/999/read`
  - 预期结果：操作失败，返回公告不存在的错误信息

- **用例4**：未登录用户标记公告
  - 请求：`POST /api/announcements/2/read`（不带token）
  - 预期结果：返回401错误，提示需要登录
...
- **用例1**：获取当前用户未读公告数量
  - 请求：`GET /api/announcements/unread/count`
  - 预期结果：返回当前用户未读公告数量

- **用例2**：未登录用户获取未读公告数量
  - 请求：`GET /api/announcements/unread/count`（不带token）
  - 预期结果：返回401错误，提示需要登录
...
- **用例1**：获取当前用户未读公告列表
  - 请求：`GET /api/announcements/unread`
  - 预期结果：返回当前用户的未读公告列表

- **用例2**：未登录用户获取未读公告列表
  - 请求：`GET /api/announcements/unread`（不带token）
  - 预期结果：返回401错误，提示需要登录
...
- **用例1**：获取当前用户已读公告列表
  - 请求：`GET /api/announcements/read`
  - 预期结果：返回当前用户的已读公告列表

- **用例2**：未登录用户获取已读公告列表
  - 请求：`GET /api/announcements/read`（不带token）
  - 预期结果：返回401错误，提示需要登录
